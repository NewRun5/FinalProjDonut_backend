...
@Configuration
public class LocalDateScalarConfig {

    /*
    * GraphQL 은 기본적으로  Int, Float, String, Boolean, ID(문자열 or 숫자) 타입을 지원한다.
    * 그 외의 타입 (Map, LocalDate...) 을 사용하기 위해선 커스텀 스칼라 타입을 지정해줘야한다.
    * */
    ...
    public GraphQLScalarType localDateScalar() {
        return GraphQLScalarType.newScalar()
                .name("LocalDate") // 1. 이름 명시. 이름은 graphql 파일에서 사용됨.
                .description("LocalDate Scalar") // 2. 설명. 설명은 문서화 된 타입에서 표현됨.
                .coercing(dateTimeCoercing())   // 3. 변환 과정 명시. 아래 코드에서 생성되는 객체.
                .build();
    }

    /* 변환 과정을 명시해주는 객체 */
    private Coercing<LocalDate, String> dateTimeCoercing() {
        return new Coercing<LocalDate, String>() {
            /**
